...
export class Traveler {
  ...
  public static travelTo(creep: Creep, destination: HasPos | RoomPosition, options: TravelToOptions = {}): number {
    ...
    // TO DO:handle case where creep moved by some other function, but destination is still the same
  ...
  public static findTravelPath(
    ...
    if (ret.incomplete && options.ensurePath) {
      if (options.useFindRoute === undefined) {
        ...
        // TO DO: handle case where a wall or some other obstacle is blocking the exit assumed by findRoute
      } else {
      }
    }
