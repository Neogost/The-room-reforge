...
/**
 * @description Manages the actions linked to the sheet has action destinations linked.
 * @author kevin desmay
 * @version 1.0
 */
export class FlagScouted {
  /**
   * @description Execution all case who are linked to the management.
  ...
  public static execute(flag: Flag) {
    ...
    switch (flag.secondaryColor) {
      case COLOR_GREEN:
        resultOK = this.add(flag.name);
        break;
      case COLOR_RED:
        resultOK = this.remove(flag.name);
      default:
        break;
    }
    if (resultOK) {
  ...
  /**
   * @description Remove a targeted room to a home room who will be scouted when needed
   * @param flagName Name fo the flag
   * @returns true if the action is correct, else false
   */
  private static remove(flagName: string): boolean {
    let result = false;
    ...
    if (roomHome && targetRoomName) {
      ...
      if (room && room.memory.linked) {
        delete room.memory.linked[targetRoomName.toString()];
      }
    }
    return result;
  }
   ...
   * @returns true if the action is correct, else false
   */
  private static add(flagName: string): boolean {
    let result = false;
    // Extract value from the flag name
    let roomHome = flagName.match(PATTERN_ORIGIN_ROOM);
    let targetRoomName = flagName.match(PATTERN_TARGET_ROOM);
    ...
    if (roomHome && targetRoomName) {
      let room = Game.rooms[roomHome.toString()];
      if (room && room.memory.linked) {
        _.set(room.memory, ["linked", targetRoomName.toString()], {
